import {
  ...
  setGridSnapAktif,
  gorunumAyarlari,
  gorunumAyariniDegistir,
} from "../core/state.js";

import { canvas, stage, viewport, gridKatmani } from "../core/stage.js";
...
export function gridiCiz() {
  ...
  const baslangicX = Math.floor(solUst.x / kucukAdim) * kucukAdim;

  const bitisX = Math.ceil(sagAlt.x / kucukAdim) * kucukAdim;

  const baslangicY = Math.floor(solUst.y / kucukAdim) * kucukAdim;

  const bitisY = Math.ceil(sagAlt.y / kucukAdim) * kucukAdim;
  ...
  for (let x = baslangicX; x <= bitisX; x += kucukAdim) {
    grafik.moveTo(x, baslangicY).lineTo(x, bitisY);
  }

  for (let y = baslangicY; y <= bitisY; y += kucukAdim) {
  ...
  const buyukBaslangicX = Math.floor(solUst.x / buyukAdim) * buyukAdim;

  const buyukBitisX = Math.ceil(sagAlt.x / buyukAdim) * buyukAdim;

  const buyukBaslangicY = Math.floor(solUst.y / buyukAdim) * buyukAdim;

  const buyukBitisY = Math.ceil(sagAlt.y / buyukAdim) * buyukAdim;

  grafik
    .beginStroke("rgba(124, 58, 237, 0.14)")
    .setStrokeStyle(1 / viewport.scaleX);

  for (let x = buyukBaslangicX; x <= buyukBitisX; x += buyukAdim) {
    grafik.moveTo(x, buyukBaslangicY).lineTo(x, buyukBitisY);
  }

  for (let y = buyukBaslangicY; y <= buyukBitisY; y += buyukAdim) {
...
export function gridNoktasinaSnap(x, y) {
  ...
  const dunyaMesafesi = Math.hypot(gridX - x, gridY - y);

  // Grid snap sınırı ekranda her zoom seviyesinde
  // yaklaşık aynı piksel mesafesinde kalsın.
  const ekranMesafesi = dunyaMesafesi * viewport.scaleX;

  if (ekranMesafesi <= GRID_SNAP_EKRAN_MESAFESI) {
    return {
      x: gridX,
      y: gridY,
...
export function odaAdlariGorunumunuDegistir() {
  gorunumAyariniDegistir("odaAdlari");
  odaAdlariButonunuGuncelle();
  ekraniGuncelle();
}

export function odaAdlariButonunuGuncelle() {
  const odaAdlariButonu = document.getElementById("btnToggleRoomNames");

  if (!odaAdlariButonu) {
    return;
  }

  odaAdlariButonu.textContent = gorunumAyarlari.odaAdlari
    ? "🏷 Oda Adları: Açık"
    : "🏷 Oda Adları: Kapalı";

  odaAdlariButonu.setAttribute(
    "aria-pressed",
    String(gorunumAyarlari.odaAdlari),
  );
}

export function gridButonlariniGuncelle() {
  const gridButonu = document.getElementById("btnGrid");

  const snapButonu = document.getElementById("btnGridSnap");

  if (gridButonu) {
    gridButonu.textContent = gridGorunur ? "▦ Grid: Açık" : "▦ Grid: Kapalı";
  }
  ...
  if (snapButonu) {
    ...
    snapButonu.classList.toggle("opacity-50", !gridGorunur);
  }
}

document
  .getElementById("btnGrid")
  ?.addEventListener("click", gridGorunumunuDegistir);

document
  .getElementById("btnGridSnap")
  ?.addEventListener("click", gridSnapDegistir);

document
  .getElementById("btnToggleRoomNames")
  ?.addEventListener("click", odaAdlariGorunumunuDegistir);
